...
export class AssetClass {
  ...
  get nine_slice(): NineSlice | undefined {
    return this._data.nine_slice;
  }
  ...
  get tile_animation(): TileAnimation | undefined {
    return this._data.tile_animation;
  }
   ...
   */
  addLayer(
    layer: Pick<Layer, 'name' | 'type' | 'opacity' | 'visible'> &
      Partial<{ physics_layer: number; role: string }>,
    parentId?: number,
    index?: number,
  ): number {
  ...
  reorderLayer(id: number, newParentId: number | undefined, newIndex: number): void {
    ...
    if (newParentId !== undefined) {
      ...
      while (curId !== undefined) {
        if (curId === id) {
          throw new Error(
            errors.invalidArgument(`Cannot move layer ${String(id)} into its own descendant`)
              .content[0].text,
          );
        }
        ...
        if (!pLayer || pLayer.type !== 'group') {
          throw new Error(
            errors.invalidArgument(`Invalid parent layer ${String(curId)}`).content[0].text,
          );
        }
  ...
  addFrame(frame: Frame, index?: number): number {
    const insertAt =
      index !== undefined && index >= 0 && index <= this._data.frames.length
        ? index
  ...
  removeFrame(index: number): void {
    if (index < 0 || index >= this._data.frames.length) {
      throw new Error(
        errors.frameOutOfRange(index, this.name, this._data.frames.length).content[0].text,
      );
    }
  ...
  addTag(tag: Tag): void {
    if (tag.type === 'frame') {
      ...
      if (ftag.start < 0 || ftag.end >= this._data.frames.length || ftag.start > ftag.end) {
        throw new Error(
          errors.invalidArgument('Frame tag bounds are out of range').content[0].text,
        );
      }
      // Check for name/facing collision
      const collision = this._data.tags.find(
        (t) => t.type === 'frame' && t.name === ftag.name && t.facing === ftag.facing,
      );
      if (collision) {
        throw new Error(
          errors.invalidArgument(
            `Frame tag with name '${ftag.name}' and facing '${ftag.facing ?? 'none'}' already exists`,
          ).content[0].text,
        );
      }
    }
    ...
    for (const key of Object.keys(this._data.cels)) {
      ...
      if ('x' in cel && 'y' in cel && 'data' in cel) {
        ...
        if (dWidth > 0 && dHeight > 0) {
          ...
          if (cx + (endX - startX) > width) {
            endX = startX + (width - cx);
          }
          if (cy + (endY - startY) > height) {
            endY = startY + (height - cy);
          }
  ...
  public _restoreDataPatch(patch: Partial<Asset>): void {
    ...
    if ('tile_terrain' in patch)
      this._data.tile_terrain = patch.tile_terrain
        ...
        : undefined;
    if ('nine_slice' in patch)
      this._data.nine_slice = patch.nine_slice
        ? (JSON.parse(JSON.stringify(patch.nine_slice)) as NineSlice)
        : undefined;
