...
export class ProjectClass {
  ...
  renameAsset(oldName: string, newName: string): void {
    ...
    if (existingNew !== undefined) {
      throw new Error(
        errors.invalidArgument(`Asset '${newName}' already exists in the registry`).content[0].text,
      );
    }
    ...
    if (entry.variants !== undefined) {
      if (variant !== undefined) {
        if (!(variant in entry.variants)) {
          throw new Error(
            errors.invalidArgument(`Variant '${variant}' not found for asset '${name}'`).content[0]
              .text,
          );
        }
        ...
        if (keys.length === 0) {
          throw new Error(
            errors.invalidArgument(`Asset '${name}' has empty variants map`).content[0].text,
          );
        }
      ...
      if (variant !== undefined) {
        throw new Error(
          errors.invalidArgument(`Asset '${name}' does not use variants`).content[0].text,
        );
      }
      if (entry.path === undefined) {
        throw new Error(
          errors.invalidArgument(`Asset '${name}' is missing path configuration`).content[0].text,
        );
      }
      relPath = entry.path;
    }
