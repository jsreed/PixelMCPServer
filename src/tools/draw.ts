...
type DrawInput = z.infer<typeof drawInputZodSchema>;

// ---------------------------------------------------------------------------
// Core draw logic (extracted for reuse across single-frame and frame-range)
// ---------------------------------------------------------------------------

function executeDrawOpsOnFrame(
  asset: AssetClass,
  layerId: number,
  frameIndex: number,
  operations: DrawInput['operations'],
  ...
  while (data.length < h) data.push(new Array<number>(w).fill(0));
  ...
  for (const op of operations) {
    switch (op.action) {
      ...
      case 'iso_tile': {
        ...
        const origin = isoToPixel(op.col, op.row, op.elevation, tw, th);
      ...
      case 'fill': {
        if (activeSelection) {
          ...
          if (
            seedSx < 0 ||
            seedSx >= activeSelection.width ||
            seedSy < 0 ||
            seedSy >= activeSelection.height ||
        ...
        const points = floodFill(op.x, op.y, w, h, (x, y) => {
          ...
          if (activeSelection) {
            ...
            if (
              sx < 0 ||
              sx >= activeSelection.width ||
              sy < 0 ||
              sy >= activeSelection.height ||
    ...
    (args: DrawInput) => {
      ...
      if (layer.type !== 'image') {
        return errors.domainError(
          `Layer ${String(layerId)} is not an image layer. Draw operations require an image layer.`,
        );
      }
      ...
      if (args.frame_range) {
        ...
        try {
          const cmd = new FrameRangeCommand(asset, layerId, start, end, (fi) => {
            executeDrawOpsOnFrame(asset, layerId, fi, args.operations, workspace, assetName);
          });
          ...
          content: [
            {
              type: 'text',
              text: JSON.stringify({
                message: `Applied ${String(args.operations.length)} drawing operations across frames [${String(start)}, ${String(end)}].`,
              }),
            },
            createResourceLink(
              assetName,
