          ...
          for (const op of args.operations) {
            switch (op.action) {
              ...
              case 'iso_tile': {
                ...
                if (!tw || !th)
                  throw new Error('iso_tile requires asset tile_width and tile_height.');
              ...
              case 'iso_cube': {
                ...
                const th = asset.tile_height;
                if (!tw || !th)
                  throw new Error('iso_cube requires asset tile_width and tile_height.');
              ...
              case 'iso_wall': {
                ...
                const th = asset.tile_height;
                if (!tw || !th)
                  throw new Error('iso_wall requires asset tile_width and tile_height.');
                const elev = 0;
