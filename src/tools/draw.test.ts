...
describe('draw tool — isometric operations', () => {
    ...
    function buildIsoAsset() {
        return AssetClass.fromJSON(({
            ...
            tags: [],
            cels: {
                '1/0': { x: 0, y: 0, data: Array.from({ length: 20 }, () => new Array<number>(20).fill(0)) },
            },
        } as unknown as Parameters<typeof AssetClass.fromJSON>[0]));
    }
    ...
    it('iso_tile returns error on a flat asset', async () => {
        ...
        const flatAsset = AssetClass.fromJSON(({
            ...
            frames: [{ index: 0, duration_ms: 100 }],
            tags: [],
            cels: { '1/0': { x: 0, y: 0, data: Array.from({ length: 8 }, () => new Array<number>(8).fill(0)) } },
        } as unknown as Parameters<typeof AssetClass.fromJSON>[0]));
    ...
    it('iso_cube paints distinct pixels with three different colors', async () => {
        const r = (await handler({
            ...
            frame_index: 0,
            operations: [
                { action: 'iso_cube', col: 2, row: 2, elevation: 0, top_color: 3, left_color: 4, right_color: 5 },
            ],
        })) as HandlerResult;
    ...
    it('iso_wall length=2 along x paints more pixels than length=1', async () => {
        ...
        const before1 = AssetClass.fromJSON(({
            ...
            frames: [{ index: 0, duration_ms: 100 }],
            tags: [],
            cels: { '1/0': { x: 0, y: 0, data: Array.from({ length: 40 }, () => new Array<number>(40).fill(0)) } },
        } as unknown as Parameters<typeof AssetClass.fromJSON>[0]));
        ...
        await handler({
            ...
            layer_id: 1,
            frame_index: 0,
            operations: [{ action: 'iso_wall', col: 2, row: 2, length: 1, axis: 'x', height: 2, color: 7 }],
        });
        ...
        await handler({
            ...
            layer_id: 1,
            frame_index: 0,
            operations: [{ action: 'iso_wall', col: 2, row: 2, length: 2, axis: 'x', height: 2, color: 7 }],
        });
