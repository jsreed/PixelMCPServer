...
describe('selection tool', () => {
    ...
    it('returns error if target asset is not loaded', async () => {
        const r = await handle({ action: 'rect', asset_name: 'missing', x: 0, y: 0, width: 2, height: 2 }) as any;
        expect(r.isError).toBe(true);
        expect(r.content[0].text).toContain('not loaded');
    });

    it('creates a rectangular selection mask', async () => {
        const r = await handle({ action: 'rect', asset_name: 'test_asset', x: 2, y: 2, width: 3, height: 3 }) as any;
    ...
    it('clears selection if rect is out of bounds', async () => {
        workspace.selection = { asset_name: 'test_asset', layer_id: 0, frame_index: 0, x: 0, y: 0, width: 1, height: 1, mask: [[true]] };
    ...
    it('pastes clipboard with offsets and wraps in undo stack', async () => {
        ...
        const r = await handle({ action: 'paste', offset_x: 4, offset_y: 4, target_asset_name: 'test_asset' }) as any;
        expect(r.isError).toBeUndefined();
