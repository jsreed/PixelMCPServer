    ...
    server.registerTool(
        ...
        async (args) => {
            ...
            if (args.action === 'clear') {
                ...
                return {
                    content: [{ type: 'text' as const, text: JSON.stringify({ message: "Selection cleared." }) }]
                };
            }
            ...
            if (args.action === 'paste') {
                ...
                const targetAsset = args.target_asset_name ?? (workspace.loadedAssets.keys().next().value as string | undefined);
                ...
                try {
                        ...
                        for (let dy = 0; dy < clip.height; dy++) {
                            for (let dx = 0; dx < clip.width; dx++) {
                                ...
                                if (px >= 0 && px < w && py >= 0 && py < h) {
                                    ...
                                    if (color !== 0) { // Assuming 0 is transparent/empty for clipboard drops
                                        data[py][px] = color;
                                    }
                                }
                            }
                    ...
                    return {
                        content: [{ type: 'text' as const, text: JSON.stringify({ message: `Pasted clipboard to layer ${layerId} frame ${frameIndex} at (${pasteX}, ${pasteY}).` }) }]
                    };
                } catch (e: any) {
            ...
            const assetName = (args as any).asset_name ?? (workspace.loadedAssets.keys().next().value as string | undefined);
            ...
            switch (args.action) {
                case 'rect': {
                    ...
                    if (width <= 0 || height <= 0 || x >= maskW || y >= maskH || x + width <= 0 || y + height <= 0) {
                        workspace.selection = null;
                    } else {
                        // Clamp bounds
                        ...
                        if (cw <= 0 || ch <= 0) {
                            ...
                            workspace.selection = { asset_name: assetName, layer_id: layerId, frame_index: frameIndex, x: cx, y: cy, width: cw, height: ch, mask };
                        }
                    }
                    return { content: [{ type: 'text' as const, text: JSON.stringify({ message: 'Rectangular selection applied.' }) }] };
                }
                case 'all': {
                    const mask = Array.from({ length: maskH }, () => new Array(maskW).fill(true));
                    workspace.selection = { asset_name: assetName, layer_id: layerId, frame_index: frameIndex, x: 0, y: 0, width: maskW, height: maskH, mask };
                    return { content: [{ type: 'text' as const, text: JSON.stringify({ message: 'Selected all.' }) }] };
                }
                case 'invert': {
                    let newMask: boolean[][];
                    if (!workspace.selection || workspace.selection.asset_name !== assetName || workspace.selection.layer_id !== layerId || workspace.selection.frame_index !== frameIndex) {
                    ...
                    workspace.selection = { asset_name: assetName, layer_id: layerId, frame_index: frameIndex, x: 0, y: 0, width: maskW, height: maskH, mask: newMask };
                    return { content: [{ type: 'text' as const, text: JSON.stringify({ message: 'Selection inverted.' }) }] };
                }
                case 'by_color': {
                    const targetColor = args.color;
                    const cel = asset.getCel(layerId, frameIndex);

                    let newMask: boolean[][] = Array.from({ length: maskH }, () => new Array(maskW).fill(false));
                    let found = false;
                    ...
                    return { content: [{ type: 'text' as const, text: JSON.stringify({ message: `Selection by color ${targetColor} complete.` }) }] };
                }
                case 'copy': {
                    const s = workspace.selection;
                    if (!s || s.asset_name !== assetName || s.layer_id !== layerId || s.frame_index !== frameIndex) {
                        return errors.domainError('No active selection to copy from this layer/frame.');
                    }

                    const cel = asset.getCel(layerId, frameIndex);
                    const clipData: number[][] = Array.from({ length: s.height }, () => new Array(s.width).fill(0));
                    ...
                    return { content: [{ type: 'text' as const, text: JSON.stringify({ message: 'Copied to clipboard.' }) }] };
                }
                case 'cut': {
                    const s = workspace.selection;
                    if (!s || s.asset_name !== assetName || s.layer_id !== layerId || s.frame_index !== frameIndex) {
                    ...
                    return { content: [{ type: 'text' as const, text: JSON.stringify({ message: 'Cut to clipboard.' }) }] };
                }
            }
