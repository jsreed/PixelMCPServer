...
describe('Minimum Viable Loop Integration', () => {
    ...
    describe('Schema Validation & Dispatch results', () => {
        it('rejects malformed tool input via Zod', async () => {
            ...
            await expect(dispatch('asset', { action: 'create', width: 'invalid_type' })).rejects.toThrow();
        ...
        it('returns expected error shapes when domain constraints fail', async () => {
            ...
            const result = await dispatch('workspace', { action: 'load_asset', asset_name: 'dummy' }) as any;
    ...
    describe('End-to-End Workflow', () => {
        it('executes Minimum Viable Loop (init -> create -> set -> draw -> verify -> save)', async () => {
            ...
            let res = unwrap(await dispatch('asset', {
                action: 'create',
                name: 'hero',
                width: 16,
                height: 16,
            ...
            // 4. Draw Operations
            res = unwrap(await dispatch('draw', {
                asset_name: 'hero',
            ...
            res = unwrap(await dispatch('asset', {
                action: 'get_cel',
                asset_name: 'hero',
                layer_id: 1,
                frame_index: 0
        ...
        it('supports undo/redo across multiple operation types', async () => {
            unwrap(await dispatch('project', { action: 'init', path: '/tmp/test_project', name: 'Test' }));
            unwrap(await dispatch('asset', { action: 'create', name: 'hero', width: 4, height: 4, palette: [[0, 0, 0, 0], [255, 255, 255, 255]] }));

            // Draw a pixel
            unwrap(await dispatch('draw', {
                asset_name: 'hero', layer_id: 1, frame_index: 0,
                operations: [{ action: 'pixel', x: 0, y: 0, color: 1 }]
            }));

            // Verify
            let res = unwrap(await dispatch('asset', { action: 'get_cel', asset_name: 'hero', layer_id: 1, frame_index: 0 }));
            ...
            res = unwrap(await dispatch('asset', { action: 'get_cel', asset_name: 'hero', layer_id: 1, frame_index: 0 }));
