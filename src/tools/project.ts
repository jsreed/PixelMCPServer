...
const projectInputSchema = {
  ...
  path: z
    .string()
    .optional()
    .describe('For init: project directory path. For open: path to pixelmcp.json'),
  name: z
    .string()
    .optional()
    .describe(
  ...
  import_path: z
    .string()
    .optional()
    .describe('For add_file: path to the PNG file to import (absolute or relative to cwd).'),
  type: z
    .string()
    .optional()
    .describe(
...
export function registerProjectTool(server: McpServer): void {
  server.registerTool(
    'project',
    {
      title: 'Project',
      description:
        'Manage on-disk project configuration and asset registry. Actions: init, open, info.',
...
async function handleAddFile(
  ...
  if (!assetName) {
    return errors.invalidArgument(
      'project add_file requires a "name" (logical asset registry name).',
    );
  }
  if (!importPath) {
  ...
  try {
    ...
    return errors.domainError(
      `Failed to save asset file: ${e instanceof Error ? e.message : String(e)}`,
    );
  }
