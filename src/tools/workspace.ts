...
const workspaceInputSchema = {
    action: z.enum(['info', 'load_asset', 'unload_asset', 'save', 'save_all', 'undo', 'redo']).describe(
        'Action to perform on the workspace session'
    ),
    asset_name: z.string().optional().describe(
        'Logical asset name (required for load_asset, unload_asset, save)'
    ),
    variant: z.string().optional().describe(
        'Variant key for multi-variant assets (used with load_asset)'
    ),
};
...
export function registerWorkspaceTool(server: McpServer): void {
    server.registerTool(
        ...
        {
            title: 'Workspace',
            description: 'In-memory editing session management. Load/unload assets, save, undo/redo, and query session state.',
...
function handleUndo(workspace: Workspace) {
    ...
    return {
        content: [{
            type: 'text' as const,
            text: JSON.stringify({ message: 'Undo successful.', undoDepth: workspace.undoDepth, redoDepth: workspace.redoDepth }),
        }],
    };
}
...
function handleRedo(workspace: Workspace) {
    ...
    return {
        content: [{
            type: 'text' as const,
            text: JSON.stringify({ message: 'Redo successful.', undoDepth: workspace.undoDepth, redoDepth: workspace.redoDepth }),
        }],
    };
}
