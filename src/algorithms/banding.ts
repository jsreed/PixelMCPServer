...
export function detectBanding(
    ...
    const addRegion = (x: number, y: number, w: number, h: number, numBands: number, contrast: number, direction: 'horizontal' | 'vertical') => {
    ...
    const evaluateRuns = (runs: Run[], isHorizontal: boolean, staticCoord: number) => {
        ...
        for (let i = 1; i < runs.length; i++) {
            ...
            const isMonotonic = (pDelta === 0 && currentDelta !== 0) || (Math.sign(currentDelta) === Math.sign(pDelta));
            const isRigidStaircase = curr.length >= MIN_RUN_LENGTH && prev.length >= MIN_RUN_LENGTH && Math.abs(curr.length - lastRunLength) <= 2; // Allow slight jitter in width

            if (isMonotonic && isRigidStaircase && Math.abs(currentDelta) < 10) { // Enforce it's a gradient (colors are close in index value)
